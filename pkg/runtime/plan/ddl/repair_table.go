...
func (c *RepairTablePlan) ExecIn(ctx context.Context, conn proto.VConn) (proto.Result, error) {
	...
	ctx, span := plan.Tracer.Start(ctx, "RepairTable.ExecIn")
	defer span.End()
	...
	if err := c.Stmt.Restore(ast.RestoreDefault, &sb, &args); err != nil {
		return nil, errors.Wrap(err, "failed to execute REPAIR TABLE statement")
	}
...
func (c *RepairTablePlan) tableReplace(tb string) {
	if tb == "" {
		return
	}
...
func (c *RepairTablePlan) physicalToLogicTable(tbName string) map[string]string {
