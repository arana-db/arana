...
func (a *OptimizeTablePlan) ExecIn(ctx context.Context, conn proto.VConn) (proto.Result, error) {
	...
	if err := a.Stmt.Restore(ast.RestoreDefault, &sb, &args); err != nil {
		return nil, errors.Wrap(err, "failed to execute Optimize TABLE statement")
	}
...
func (a *OptimizeTablePlan) tableReplace(tb string) {
	if tb == "" {
		return
	}
	...
	stmt := ast.NewOptimizeTableStatement()
...
func (a *OptimizeTablePlan) physicalToLogicTable(tbName string) map[string]string {
